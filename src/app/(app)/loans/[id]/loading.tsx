...
import { Edit3, ListChecks as RecordIcon, CircleDollarSign, Repeat, Wallet, ListChecks, ReceiptText, Percent, Download } from 'lucide-react';
      ...
      <div className="shadow-lg bg-card rounded-xl">
        <div className="p-6">
          <div className="flex justify-between items-start gap-2 mb-1">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <Skeleton className="h-9 w-32 rounded-md" /> {/* Export CSV Button */}
            </div>
          </div>
        ...
        <div className="p-6 pt-0 space-y-6">
            ...
            {[1, 2, 3, 4, 5, 6].map((i) => ( // Now 6 cards
              <div key={i} className="shadow-md rounded-xl bg-card p-4">
                <div className="flex flex-row items-center justify-between space-y-0 pb-2">
                  ...
                  {i === 6 && <Percent className="h-4 w-4 text-muted-foreground" />} {/* New card icon */}
                </div>
                <div>
                  <Skeleton className="h-7 w-3/4 rounded" /> {/* Metric Value */}
                  {(i === 4 || i === 5 || i === 6) && <Skeleton className="h-3 w-1/2 mt-1 rounded" />} {/* Sub-text */}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-sm">
            {[...Array(7)].map((_, i) => ( // 7 items for other details
